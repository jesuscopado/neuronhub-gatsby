...
const AuthorHero: React.FC<AuthorHeroProps> = ({ author }) => {
  ...
  const results = useStaticQuery(longBioQuery);
  const longBio = results.site.edges.find(a => a.node.id === author.id).node.longBio;
  const bio = longBio ? longBio : author.bio;
